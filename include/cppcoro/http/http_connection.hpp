...
namespace cppcoro::http
{
	...
	template<
		typename ParentT,
		net::is_connection_socket_provider SocketProviderT,
		typename ResponseT = string_response,
		typename RequestT = string_request>
	class connection : public tcp::connection<typename SocketProviderT::connection_socket_type>
	{
	...
	public:
		...
		explicit connection(server<SockProviderT>& server, tcp::connection<socket_type> connection)
			: tcp::connection<socket_type>(std::move(connection))
			, parent_{ server }
			, /*input_{std::make_unique<request>()},*/
			buffer_(2048, 0)
		{
		...
		explicit connection(client<SockProviderT>& client, tcp::connection<socket_type> connection)
			: tcp::connection<socket_type>(std::move(connection))
			, parent_{ client }
			, /*input_{std::make_unique<response>()},*/
			buffer_(2048, 0)
		{
	...
	private:
		...
		_send(std::string&& path, std::string&& data, http::headers&& headers) requires(is_client())
		{
			...
			auto resp = co_await next(
				[&](http::response_parser& parser) -> receive_type& {
                    response = parser;
					return response;
				});
			if (resp)
