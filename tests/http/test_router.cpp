...
{
    struct session_t { int id = 42; } session;
    router::router router {std::make_tuple(std::ref(session)),
                           http::route::get<R"(/hello/(\w+))">([](const std::string& who, cppcoro::router::context<session_t> session) {
                             return fmt::format("Hello {} from {} !", who, session->id);
                           }),
                           http::route::get<R"(/add/(\d+)/(\d+))">([](int lhs, int rhs) {
                             return lhs + rhs;
                           })
    };
    REQUIRE(std::get<std::string>(router("/hello/world", http::method::get)) == "Hello world from 42 !");
    REQUIRE(std::get<std::string>(router("/hello/world", http::method::post)) == "");
}

SCENARIO("http::serve can handle router", "[cppcoro-http][router]")
{
    struct session_t { int id = 42; } session;
    router::router router {std::make_tuple(std::ref(session)),
                           http::route::get<R"(/hello/(\w+))">([](const std::string& who, cppcoro::router::context<session_t> session) {
